...
const NAV_ITEMS = [
  ...
  { href: '/condos',     label: 'Condominios', icon: 'C' },
  { href: '/checkout',   label: 'Checkout',    icon: 'X' },
      ...
      <nav className="flex-1 py-2 overflow-y-auto">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <span className="text-[14px] w-5 text-center font-mono">{item.icon}</span>
              {item.label}
            </Link>
          );
        })}
